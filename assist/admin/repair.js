...
// import * as partOfSpeech from './dictionary.pos.js';
...
export default async function(q){
  ...
  raw = await db.mysql.query('SELECT id, word, sense FROM ?? WHERE sense LIKE ?;',[table.senses,'%\t%']).then(e=>e).catch(
    e=>{
  ...
  for (let row of raw) {
    ...
    row.sense = chat.stripString(formatParenthesisWithBold(row.sense));
...
const createParenthesisWithBold = function(str){
  ...
  return str.replace(/([\[\(\{]?(\s|;|;\s?~|~:|~\s?|-)?\w+[\s\/\-\,~\:\;\.\']?[\]\)\}]?)+/g,(fullMatch)=>{
    ...
    var testString = testBracket.filter(
      ...
      e => new RegExp(e[0]).test(fullMatch)
    );
...
const formatParenthesisWithBold = function(str){
  ...
  abc.filter(
    e => new RegExp(e[0]).test(str)
  ).forEach(function(e){
    str = str.replace(e[0],e[1].toString());
  });
...
const updateSense = async function(row){
  await db.mysql.query('UPDATE ?? SET sense = ? WHERE id = ?;',[table.senses,row.sense,row.id]).then(
...
const update2NULL = async function(row){
  ...
  return await db.mysql.query('UPDATE ?? SET word = NULL, tid = 0, sense = NULL, exam = NULL, seq = 0, kid = 0, wid = 0 WHERE `word` LIKE ?;',[table.senses,row.word]).then(
